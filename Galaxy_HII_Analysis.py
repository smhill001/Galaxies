...
This function produces plots of HII distributions in galaxies. It uses 
...
def Galaxy_HII_Regions(drive,Target,TargetType):
    import pylab as pl
    import numpy as np
    ...
    #Valid for M33, M81, and M101
    #Example call:
    #Galaxy_HII_Regions("f:","M101","Galaxies")
    
    path=drive+"/Astronomy/Python Play/Galaxies/"
    
    
    #Parameter Initialization
    HISTparams=PU.PlotSetup(path+"HIIPlotConfig.txt")
    HISTparams.loadplotparams(drive,Target,"POSHist")
    DENSparams=PU.PlotSetup(path+"HIIPlotConfig.txt")
    DENSparams.loadplotparams(drive,Target,"POSDens")
    LOCparams=PU.PlotSetup(path+"HIIPlotConfig.txt")
    LOCparams.loadplotparams(drive,Target,"POSLoc")
    
    GXparams=CF.Target_Parameters(path+"Galaxy_Parameters.txt")
    GXparams.loadtargetparams(Target)
    ...
    DataList.loadcatalog()
    print DataList.RAJ2000
